...
async def get_settings():
    return {
        ...
        "telegram": {
            ...
            "TG_BOT_TOKEN": bool(S.TG_BOT_TOKEN),  # do not leak value
        },
    }
...
async def patch_branding(payload: BrandingIn):
    ...
    await st.set_async("branding.PUBLIC_URL", payload.PUBLIC_URL)
    await st.set_async("branding.BOT_LANDING_URL", payload.BOT_LANDING_URL)
    # warm cache for immediate effect
    st._cache["branding.BRAND_NAME"] = payload.BRAND_NAME
    st._cache["branding.PUBLIC_URL"] = payload.PUBLIC_URL
...
async def patch_telegram(payload: TelegramIn):
    ...
    if payload.TG_BOT_USERNAME is not None:
        await st.set_async("telegram.TG_BOT_USERNAME", payload.TG_BOT_USERNAME.lstrip("@"))
    if payload.TG_BOT_TOKEN:
        await st.set_async("telegram.TG_BOT_TOKEN", payload.TG_BOT_TOKEN, is_secret=True)
    st._cache["telegram.TG_LOGIN_ENABLED"] = "1" if payload.TG_LOGIN_ENABLED else "0"
    if payload.TG_BOT_USERNAME is not None:
        st._cache["telegram.TG_BOT_USERNAME"] = payload.TG_BOT_USERNAME.lstrip("@")
    if payload.TG_BOT_TOKEN:
        # do not cache secrets if encryption enabled; keep placeholder True
        st._cache["telegram.TG_BOT_TOKEN"] = "***"
    return {"ok": True}
