...
export type AreasManagerVariant = 'page' | 'settings';
...
export default function AreasManager({ variant = 'page' }: AreasManagerProps) {
  ...
  const containerClassName =
    variant === 'page'
      ? 'grid gap-6 lg:grid-cols-[320px,minmax(0,1fr)]'
      : 'grid gap-6 lg:grid-cols-[minmax(0,300px),minmax(0,1fr)]';
        ...
        <div className="rounded-2xl border border-subtle bg-surface-soft p-5">
          ...
          {isLoading ? (
            ...
            <p className="mt-4 text-sm text-muted">
              Пока нет областей — создайте первую область для структуры PARA.
            </p>
          ) : (
                    ...
                    <button
                      ...
                      className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm transition-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface-0)] ${
                        isActive
                          ...
                          : 'text-[var(--text-primary)] hover:bg-[var(--surface-0)]'
                      }`}
        ...
        <div className="rounded-2xl border border-subtle bg-[var(--surface-0)] p-6">
          {!selectedArea ? (
            ...
            <div className="flex flex-col gap-5">
              ...
              {detailMessage ? (
                <div className="text-sm text-emerald-600">{detailMessage}</div>
              ) : null}
              {detailError ? <div className="text-sm text-red-500">{detailError}</div> : null}
              <p className="text-xs text-muted">
                Напоминание: у каждой сущности должна быть область. Изменения мгновенно обновят фильтры в задачах,
                календаре и привычках.
              </p>
            </div>
          )}
        </div>
