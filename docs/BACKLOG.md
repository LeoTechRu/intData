...
**Tasks**
- [ ] P0•L — Спроектировать CRM-сущности (`crm_accounts`, `crm_deals`, `crm_touchpoints`) с обязательной привязкой к PARA (`area_id`/`project_id`) и описанием схемы в `core/db/ddl/*`.
- [ ] P0•M — Реализовать цифровую воронку (канбан-доску) со стадиями, автоматизациями и метриками по аналогии с amoCRM Digital Pipeline; отрисовать в `/crm/deals`.
- [ ] P0•M — Собрать контакт-центр: единый таймлайн коммуникаций (почта, чат, звонки) и живой виджет операторов, вдохновляясь Bitrix24.
- [ ] P1•M — Интегрировать Zettelkasten-принципы: атомарные заметки, бэклинки и теги для сделок/контактов, чтобы CRM выступала узлом знаний.
- [ ] P1•S — Документировать CRM-архитектуру в `docs/vision.md` (данные, автоматизации, UX) и синхронизировать Acceptance Criteria с PARA-процессами.
- [ ] P1•S — Подготовить матрицу автоматизаций и роботов (оплаты, обучение, upsell) с учётом возможностей Bitrix24 (процессы, аналитика).

**Acceptance Criteria**
- Каждая сущность CRM хранит `area_id`/`project_id`; сделки наследуют контекст PARA и отображаются в соответствующих обзорах и отчётах.
- Карточка сделки содержит панель знаний: заметки, решения, ретроспективы и бэклинки доступны в один клик.
- Канбан CRM показывает этапы, конверсию и ожидаемый доход; переход между этапами может запускать автоматизации.
- Контакт-центр объединяет входящие сообщения, звонки и ответы операторов; доступен виджет «ответить из CRM».
- Документация в `docs/vision.md` и `docs/conventions.md` описывает UX-паттерны, автоматизации и правила работы с данными CRM.
