...
#### E7b: Каталоги профилей (Users/Groups/Projects/Areas)
**User Stories**
...
**Tasks**
- [ ] P0•L — Создать унифицированную модель `entity_profiles` + `entity_profile_grants` с поддержкой типов (`user`, `group`, `project`, `area`) и аудитории (`public`, `authenticated`, `user`, `group`, `project`).
...
- [ ] P0•S — Добавить UI профиля для групп `/groups/{slug}` и проектов `/projects/{slug}`, отразить статусы приватности и CTA «Запросить доступ».
...
- `/groups/{slug}` и `/projects/{slug}` отдают профиль, соответствующий настройкам видимости, включая секцию «Контакты» и метрики (участники/прогресс).
- `/areas/{slug}` отображает карточку области с ключевыми проектами и владельцем; при отсутствии разрешения — CTA «Запросить доступ».
