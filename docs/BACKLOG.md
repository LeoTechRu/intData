...
- [x] `/habits` страница использует веб-сессию для чтения; write-действия требуют TG и отвечают `403 tg_link_required`.
- [x] Кулдаун привычек мапится на `429` с заголовком `Retry-After`.
- [x] Снимок OpenAPI включает ошибки `tg_link_required` и `cooldown`; рантайм и файл синхронизированы.
- [x] Тесты покрывают веб-сессию, `tg_link_required`, `429 Retry-After` и OpenAPI-паритет.
