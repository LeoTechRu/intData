...
### Fixed
- `/habits` доступен по веб-сессии; write требует TG (403 `tg_link_required`).
- Повторный клик привычки возвращает 429 с заголовком `Retry-After`.
- Снимок `api/openapi.json` синхронизирован с рантаймом.
