...
### Fixed
- Backend автоматически прогоняет `npm run build`, если Next.js артефакты отсутствуют, и отдаёт `/users` и `/_next/static`, поэтому каталог и профили пользователей снова открываются после миграции на новый фронтенд.
