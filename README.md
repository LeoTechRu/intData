...
### API-префикс (правило)
- Все серверные эндпоинты, возвращающие JSON/данные, доступны только под путями `\`/api/v1/...\``.
- Короткие пути без префикса зарезервированы под UI (HTML-страницы).
- Старые пути временно могут отдавать `307 Redirect` на новые `\`/api/v1/...\`` с заголовком `Deprecation: true`.
...
перенаправляются на новые адреса с кодом `308` и заголовками
`Deprecation: true` и `Sunset`. По умолчанию Sunset устанавливается на
60 дней вперёд (`API_SUNSET_DAYS`). Поведение редиректов можно отключить
через переменную `API_REDIRECTS_ENABLED=false`, тогда устаревшие пути будут
возвращать `410 Gone` с ссылкой на актуальную версию.
