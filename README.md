...
- [x] Подготовлено окружение: установлены зависимости, тесты проходят локально (SQLite); запуск веб-приложения требует PostgreSQL.
...
- [x] UI страницы доступны по коротким URL (`/tasks`, `/notes` и др.), а REST API перенесено под префикс `/api/*`.
...
### API-префикс (правило)
- Все серверные эндпоинты, возвращающие JSON/данные, доступны только под путями `\`/api/...\``.
- Короткие пути без префикса зарезервированы под UI (HTML-страницы).
- Старые пути временно могут отдавать `307 Redirect` на новые `\`/api/...\`` с заголовком `Deprecation: true`.
