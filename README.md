...
## Брендинг
- Инструкции по фирменному стилю и использованию логотипов: [AGENTS.md](AGENTS.md)
...
Intelligent Data Platform предоставляет приватный iCalendar‑фид:
...
- [x] Зафиксированы правила нейминга Intelligent Data Platform и @intDataBot в `docs/agents.md`.
- [x] Добавлен лендинг бота @intDataBot и маршрут `/bot`.
- [x] Добавлен раздел Branding в `.env.example` с переменными `BRAND_NAME`, `PUBLIC_URL`, `TG_BOT_USERNAME`, `BOT_LANDING_URL`.
...
- [x] Проверены тексты на соответствие правилам нейминга Intelligent Data Platform и @intDataBot.
...
- [x] Унифицирована работа с cookies через функцию `set_auth_cookies`.
- [x] Подготовлена базовая система миграций БД с помощью Alembic.
- [x] Дашборд наполняется реальными данными задач, напоминаний и событий; KPI и таймлайн формируются на основе сервисов.
- [x] Добавлены веб-страницы календаря и учёта времени.
 - [x] Добавлен виджет привычек на дашборде, расчёт прогресса вынесен в `core.utils.habit_utils`.
- [x] Добавлены скрипты для проверки подключения к PostgreSQL и устаревших зависимостей (`scripts/check_postgres.py`, `scripts/check_dependencies.py`).
- [x] UI страницы доступны по коротким URL (`/tasks`, `/notes` и др.), а REST API перенесено под префикс `/api/v1/*`.
- [x] Добавлены тесты для HabitService и напоминаний, повышено покрытие.
- [x] Настроен CI (GitHub Actions): запускает pytest и flake8 на PR и push.

### Следующие шаги
- [x] Настроить фоновый планировщик и рассылку уведомлений по напоминаниям (за флагом).
- [x] Покрыть новые модули тестами и настроить CI/CD.
...
- [x] Тесты и CI/CD для бота и веб-сервера
...
## Миграции
SQL-файлы лежат в `db/migrations`, применяются скриптом `dev/migrate.sh` (использует `DATABASE_URL` или `DB_*` из `.env`).
Применённые версии сохраняются в таблице `schema_migrations`.

## Веб-морда
Веб-приложение на FastAPI предоставляет админ-панель с авторизацией через Telegram.

### API-префикс (правило)
- Все серверные эндпоинты, возвращающие JSON/данные, доступны только под путями `\`/api/v1/...\`` (без завершающего `/`).
- Короткие пути без префикса зарезервированы под UI (HTML-страницы).
- Запросы к устаревшим `\`/api/*\`` возвращают `404` с подсказкой использовать `\`/api/v1/*\``.
- Swagger UI доступен на `\`/api\``; спецификация — `\`/api/openapi.json\``.

## API Versioning
API находится на первом стабильном уровне `/api/v1`. Старые JSON-пути
не обслуживаются и возвращают `404`.

### Настройка `.env`
Создайте файл `.env` в корне репозитория со следующими переменными:

```dotenv
TG_BOT_TOKEN=123456789:AA...your...token
TG_BOT_USERNAME=YourBotName   # БЕЗ @; можно оставить TG_BOT_USERNAME — тоже подхватится
PUBLIC_URL=http://109.196.99.158:5800  # можно указать домен или IP с портом
...
- «Field required: TG_BOT_USERNAME» → используйте `TG_BOT_USERNAME` или `TG_BOT_USERNAME`.
...
## Деплой и сервисы
Автодеплой на сервер настроен через workflow [deploy.yml](.github/workflows/deploy.yml). Добавьте секреты `VPS_HOST`, `VPS_USER` и `VPS_KEY` с данными SSH. При пуше в `main` репозиторий копируется в `/sd/intdata`, устанавливаются зависимости и перезапускаются сервисы.
...
Включается переменной окружения `ENABLE_SCHEDULER=1`. По умолчанию используется безопасный отправитель, который логирует уведомления без сетевых запросов. Для боевого окружения передайте собственный sender (см. `core/services/notification_service.py`) или расширьте его для отправки через Telegram-бота.

Пример запуска с флагом:
```bash
ENABLE_SCHEDULER=1 uvicorn main:app --host 0.0.0.0 --port 5800
```
