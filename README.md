...
- Боковая навигация AppShell теперь модульная: в рейле отображаются только модули (Control Hub, «Задачи и проекты», «База знаний», «Команда», «Администрирование») с возможностью свернуть их до иконок. Порядок и метаданные модулей/страниц берутся из `NAV_BLUEPRINT` (см. [Vision Deck](#-vision-deck), инициатива «Modular Navigation»).
