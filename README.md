...
- [x] UI страницы доступны по коротким URL (`/tasks`, `/notes` и др.),
      а REST API перенесено под префикс `/api/*`.

### Следующие шаги
- [x] Доработать веб-интерфейс для календаря и тайм-трекинга.
- [ ] Настроить фоновый планировщик и рассылку уведомлений по напоминаниям.
- [x] Наполнить дашборд данными через новую библиотеку виджетов.
- [x] Подключить реальные данные для KPI и таймлайна.
- [x] Реализовать управление несколькими Telegram-аккаунтами в веб-интерфейсе.
- [x] Обработать все типы чатов Telegram и права доступа.
...
- [ ] Проверить запуск приложения с PostgreSQL.
...
- [x] Отображение предстоящих задач, напоминаний и событий с возможностью быстрого взаимодействия.
 - [x] Виджет привычек с визуализацией прогресса.
...
- [x] Унифицирована работа с cookies и авторизацией для всех типов входа.
