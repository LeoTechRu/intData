...
- [x] Добавлены веб-страницы задач и заметок.
...
- [x] Добавлены методы обновления и удаления задач в сервисе `TaskService`.
...
- [x] Дашборд наполняется реальными данными задач и событий; KPI и таймлайн формируются на основе сервисов.
...
Включается переменной окружения `ENABLE_SCHEDULER=1`. При запуске активируется `ProjectNotificationWorker`, который опрашивает календарные будильники и отправляет уведомления.
