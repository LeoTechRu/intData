...
- `AGENTS.md` содержит операционные правила, Agent Sync и DevOps-процедуры; структуру каталогов см. в разделе [«Структура репозитория»](AGENTS.md#структура-репозитория).
...
## Как мы работаем
IntData живёт на одном хосте в трёх ветках: `dev` (разработка), `test` (живой тест-макет в `/git/intdata`) и `main` (стабильный макет в `/sd/intdata`). Агенты коммитят и пушат только в `dev`, Team Lead сразу после ревью мержит в `test`, а система создаёт черновик PR `test→main`.
После ручного approve владельца PR попадает в `main`, и DevOps обновляет стойки: `/git/intdata` после merge `dev→test`, `/sd/intdata` после merge `test→main`. Подробные регламенты и политика Agent Sync описаны в [AGENTS.md](AGENTS.md).

## Как собрать и запустить тест-макет
- Перейти на хосте в `/git/intdata` (ветка `test`, переключение запрещено).
- Обновить зависимости: `python -m pip install -r requirements.txt`, `npm --prefix web install`.
- Пересобрать фронтенд: `npm --prefix web run build`.
- Перезапустить сервисы под systemd: `sudo systemctl restart intdata-web intdata-bot`; если используется compose, см. команды в разделе DevOps файла [AGENTS.md](AGENTS.md).
- Выполнить smoke: `curl -f https://test.intdata.pro/healthz` и сохранить вывод в `reports/devops/<timestamp>/smoke.txt`.

## Как обновить стабильный макет `/sd/intdata`
- Убедиться, что PR `test→main` одобрен владельцем и замержен.
- Перейти в `/sd/intdata` (ветка `main`, переключение запрещено) и синхронизировать рабочие файлы (`git pull --ff-only`).
- Повторить rebuild так же, как на тестовой стойке (systemd либо compose, в зависимости от окружения).
- Перезапустить сервисы (`sudo systemctl restart intdata-web intdata-bot`) и выполнить smoke: `curl -f https://intdata.pro/healthz`.
- Логи и smoke складываем в `reports/devops/<timestamp>/`.

## Логи и сигнатуры ошибок
- Рабочие логи и сводки DevOps храним в `reports/devops/<timestamp>/`, формат UTC ISO8601.
- При просмотре логов ищем сигнатуры: `ERROR`, `FATAL`, `CRITICAL`, `Traceback`, `Unhandled`, `panic`, `OOM`, `bind: address already in use`, `Migrations failed`, `connection refused`.
- Если обнаружены ошибки, фиксируем краткую сводку и при необходимости эскалируем TL.

## Smoke и health-check
- Основные точки проверки: `https://test.intdata.pro/healthz`, `https://test.intdata.pro/readyz`, `https://intdata.pro/healthz`.
- Для автоматизации используем `curl -fsS` и сохраняем ответы в `reports/devops/<timestamp>/smoke.txt`.
- При нестабильном ответе повторяем проверку после перезапуска и сообщаем владельцу вместе с логами.
...
- [x] TL-2025-09-19-notes-assign-detached — Исправить DetachedInstanceError при `POST /api/v1/notes/{id}/assign`, убедиться что ответ содержит `area`/`project` без ленивых загрузок (owner: codex, ветка `dev` (исторический идентификатор `E3/notes-assign-detached-codex`), PR [#102](https://github.com/LeoTechRu/intData/pull/102), см. [E3](#e3-api-calendar-calendaritems-calendaragenda-calendarfeedics-projectsidnotifications)).
...
- [x] TL-2025-09-20-release-runbook — Обновить `reports/runbooks/test-to-main.md` и CI `tests.yml`/`deploy-test.yml` под фичи E2/E3/E17, подготовить smoke-чеклист для fast-forward `test->main` (owner: devops, ветка `dev` (исторический идентификатор `E9/test-branch-deploy-codex`), отчёт `reports/runbooks/test-to-main.md`).
- [x] TL-2025-09-20-release-docs — Синхронизировать README (Conventions/Tasklist/Workflow), Changelog и `reports/*` по результатам релиса E2/E3/E17, приложить ссылки на GateRecord и отчёты QA/InfoSec (owner: tw, ветка `dev` (исторический идентификатор `E2/release-docs-tw`), см. [Workflow Playbook](#-workflow-playbook)).
- [x] TL-2025-09-21-trivy-scan — Запустить CI workflow `Security Scan (Trivy)` и приложить отчёт `trivy-report.json` перед Gate-5 (owner: codex, ветка `dev` (исторический идентификатор `release/trivy-scan-devops`), отчёт `reports/infosec/trivy-2025-09-23.json`, CRITICAL/HIGH не обнаружены).
- [x] TL-2025-09-23-starlette-upgrade — Обновить `fastapi`→0.117.1 и `starlette`→0.48.0, прогнать `pytest tests/web/test_calendar_feed_ics.py tests/web/test_alarms_api.py tests/test_diagnostics_service.py`, переиздать Trivy (owner: codex, ветка `dev` (исторический идентификатор `release/starlette-upgrade-be`), отчёт `reports/infosec/trivy-2025-09-23.json`).
...
- [x] TL-2025-09-19-roles-charter — Обновить AGENTS.md и README.md: Roles Charter, auto-switch, раздел для владельца (owner: codex, ветка `dev` (исторический идентификатор `Ops/roles-charter-tw`), см. [E9](#e9-%D1%82%D0%B5%D1%81%D1%82%D1%8B-%D0%B8-%D0%B4%D0%BE%D0%BA%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D0%B0%D1%86%D0%B8%D1%8F-%D1%84%D0%B8%D1%87%D0%B5%D1%84%D0%BB%D0%B0%D0%B3)).
- [x] TL-2025-09-19-pytest-postgres-env — Настроить pytest на PostgreSQL через `TEST_DATABASE_URL`/`TEST_DB_*` в `.env`, чтобы тестовая база не конфликтовала с рантаймом (owner: codex, ветка `dev` (исторический идентификатор `E9/test-postgres-env-codex`); отчёты `reports/2025-09-20-pytest-postgres-qa.md`, `reports/2025-09-20-ci-timeouts-analysis.md`, GateRecord `reports/2025-09-20-gaterecord-e9-test-postgres.md`).
- [x] TL-2025-09-19-test-branch-deploy — Настроить ветку `test` с автоматическим деплоем в тестовый контур и запретом автоудаления (owner: codex, ветка `dev` (исторический идентификатор `E9/test-branch-deploy-codex`), workflow `.github/workflows/deploy-test.yml`, требуется настройка секретов `TEST_VPS_*`; см. [E9](#e9-%D1%82%D0%B5%D1%81%D1%82%D1%8B-%D0%B8-%D0%B4%D0%BE%D0%BA%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D0%B0%D1%86%D0%B8%D1%8F-%D1%84%D0%B8%D1%87%D1%8D%D1%84%D0%BB%D0%B0%D0%B3)).
- [x] TL-2025-09-19-test-secrets — Вынести секреты `TEST_*` (БД, URL, бот) и подготовить токен `@intDataTestBot` для тестового контура (owner: codex, ветка `dev` (исторический идентификатор `E9/test-branch-deploy-codex`), обновлены `.env.example`, workflows и документация по секретам; см. [E9](#e9-%D1%82%D0%B5%D1%81%D1%82%D1%8B-%D0%B8-%D0%B4%D0%BE%D0%BA%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D0%B0%D1%86%D0%B8%D1%8F-%D1%84%D0%B8%D1%87%D1%8D%D1%84%D0%BB%D0%B0%D0%B3)).
- [x] TL-2025-09-19-test-runbook — Описать release-runbook `test → main` и smoke-чеклист (owner: codex, ветка `dev` (исторический идентификатор `E9/test-branch-deploy-codex`), документ `reports/runbooks/test-to-main.md`, README/Changelog обновлены; см. [E9](#e9-%D1%82%D0%B5%D1%81%D1%82%D1%8B-%D0%B8-%D0%B4%D0%BE%D0%BA%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D0%B0%D1%86%D0%B8%D1%8F-%D1%84%D0%B8%D1%87%D1%8D%D1%84%D0%BB%D0%B0%D0%B3)).
- [x] TL-2025-09-19-pytest-postgres-migration — Перевести диагностические и web-тесты на PostgreSQL, устранить `MissingGreenlet` и FK-конфликты (owner: codex, ветка `dev` (исторический идентификатор `E9/test-postgres-env-codex`); отчёт `reports/2025-09-19-web-postgres-progress.txt`, follow-up: увеличить таймаут полного `pytest -q`).
...
- [x] TL-2025-09-19-notes-restore — Переработать страницу `/notes`: безопасное архивирование карточек с восстановлением, отдельный просмотр архива, синхронизация drag-n-drop и фильтров (owner: codex, ветка `dev` (исторический идентификатор `E10/notes-restore-frontend`), merge 4934acc в `test→main`).
- [x] TL-2025-09-24-notes-collapsible — Свернуть формы быстрой заметки и фильтров в раскрывающиеся секции на `/notes`, подготовить компонент CollapsibleSection и обновить тесты (owner: codex, intake `reports/2025-09-24-intake-notes-collapsible.yaml`).
  - [x] ARCH-2025-09-24-notes-collapsible — ADR и технические рамки для раскрывающихся секций, handoff FE (owner: architect, ветка `dev` (исторический идентификатор `E10/notes-collapsible-arch`)).
  - [x] FE-2025-09-24-notes-collapsible — Компонент `CollapsibleSection`, обновлённый `NotesModule`, unit-тесты (owner: frontend, ветка `dev` (исторический идентификатор `E10/notes-collapsible-fe`)).
  ...
  - [x] DEVOPS-2025-09-24-notes-collapsible — Runbook обновлён, lint/test/build прогнаны, готов к fast-forward (owner: devops, ветка `dev` (исторический идентификатор `E10/notes-collapsible-release`)).
  - [x] TW-2025-09-24-notes-collapsible — README/Agent Sync синхронизированы, GateLog закрыт (owner: tech writer, ветка `dev` (исторический идентификатор `E10/notes-collapsible-docs`)).

- [x] TL-2025-09-18-nav-blueprint — Расширить NAV_BLUEPRINT и API `/api/v1/navigation/sidebar*` полями модулей и секций (owner: codex, ветка `dev` (исторический идентификатор `E17/nav-blueprint-codex`), см. [E17](#e17-frontend-modernization)).
- [x] TL-2025-09-18-appshell-modules — Перестроить AppShell и SidebarEditor: секции + collapsible, единый список избранных страниц без дублей (owner: codex, ветка `dev` (исторический идентификатор `E17/nav-blueprint-codex`)).
- [x] TL-2025-09-18-module-tabs — Добавить верхние вкладки модулей и компонент FavoriteToggle, управляющий видимостью страниц в меню (owner: codex, ветка `dev` (исторический идентификатор `E17/nav-blueprint-codex`)).
- [x] TL-2025-09-19-appshell-nav-tuning — AppShell: компактный header на мобайле, независимый скролл, вкладки модулей справа от сайдбара и сворачиваемые секции меню с управлением страницами (owner: codex, ветка `dev` (исторический идентификатор `E17/appshell-nav-tuning-codex`)).
- [x] TL-2025-09-18-bot — Восстановить публичный лендинг `/bot` на Next.js (agent: codex, ветка `dev` (исторический идентификатор `E17/bot-landing-codex`)).
- [x] TL-2025-09-18-groups — Перенести `/groups`, `/groups/manage/{id}` и `/products` на Next.js, добавить тултипы `TermHint`, удалить legacy-шаблоны и `ui_router` (agent: codex, ветка `dev` (исторический идентификатор `E17/groups-products-ui-codex`)).
- [x] TL-2025-09-18-support — Обновить лендинг `/tariffs` (кликабельное сообщество, упоминания поддержки) и добавить условные кнопки поддержки в AppShell (agent: codex, ветка `dev` (исторический идентификатор `E17/groups-products-ui-codex`)).
- [x] TL-2025-09-18-legacy-final — Завершить перенос legacy-страниц: включить `/products` и `/groups` в AppShell, перевести `/ban` и `/cup/admin-embed` на Next.js, удалить Jinja-шаблоны и статические JS/CSS (agent: codex, ветка `dev` (исторический идентификатор `E17/legacy-migration-codex`)).
- [x] TL-2025-09-18-mobile-ui — Подточить мобильную адаптивность AppShell и дашборда (`/`): убрать чип роли на узких экранах, перестроить сетку шапки, скрыть редактор дашборда (agent: codex, ветка `dev` (исторический идентификатор `E17/mobile-responsive-ui-codex`)).
- [x] TL-2025-09-24-bitrix-smart-sidebar — Stage-gate SmartSidebar (drag-and-drop, скрытия, глобальные/персональные layout) и ModuleTabsBar без пользовательских правок; оформить ADR/layout API, TaskCards и gate-логи (owner: teamlead, ветка `main`, intake `reports/2025-09-24-intake-bitrix-smart-sidebar.yaml`).
  - [x] BE-2025-09-24-sidebar-layout — Бэкенд: таблица `nav_sidebar_layouts`, REST `GET/POST` layout, merge-утилиты, pytest `tests/web/test_navigation_layout_api.py` (owner: backend, ветка `dev` (исторический идентификатор `E17/smart-sidebar-layout-be`)).
  - [x] FE-2025-09-24-smart-sidebar — SmartSidebar UI (drag-and-drop, скрытие/возврат страниц) и ModuleTabsBar Bitrix24-like (owner: frontend, ветка `dev` (исторический идентификатор `E17/smart-sidebar-ui-fe`)).
  ...
  - [x] SEC-2025-09-24-smart-sidebar — Advisory SEC-SS-001/002/003 оформлен (owner: infosec, ветка `dev` (исторический идентификатор `E17/smart-sidebar-infosec`)).
  - [x] DEVOPS-2025-09-24-smart-sidebar — Rebuild через `scripts/rebuild_smart_sidebar.sh`, ansible deploy тестового стора, smoke desktop/mobile и лог-чек `intdata-test-web` (owner: devops, ветка `dev` (исторический идентификатор `E17/smart-sidebar-release`)).
  - [x] TW-2025-09-24-smart-sidebar — README/Agent Sync синхронизированы, GateRecord release/docs закрыт, follow-ups занесены (owner: tech writer, ветка `dev` (исторический идентификатор `E17/smart-sidebar-docs`)).
...
**Tasks**
- [x] P0•S — Машиночитаемая схема БД и автопроверка (`python -m backend.db.schema_export`, CI‑check).
...
- [ ] P2•S — Линтер `scripts/para_lint.py` и запуск в CI.
...
- Проверки доступа на веб/API используют `backend.authz` сервис с методом `require(permission, scope)`; прямые обращения к `UserRole` в HTTP-слое устранены.
...
- [ ] P0•S — Обновить `/README.md (секция «Changelog»)` и `/backend/api/openapi.json` после внедрения новых эндпоинтов/ответов.
...
- P0•M — Ветка `test`: обязательный PR-флоу (dev → test → main), автоматический деплой в тестовый контур и запрет автоудаления.
...
- [x] `api/openapi.json` совпадает с `/backend/api/openapi.json` и содержит модели ошибок `tg_link_required` и `cooldown`.
...
- Унифицирована работа с паролями через обёртку `backend.db.bcrypt` и `WebUserService`.
...
- Unified OpenAPI SSoT at `/backend/api/openapi.json`; exporter produces `api/openapi.json`.
