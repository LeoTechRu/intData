...
### Следующие шаги
- [ ] Вставить иконку paper-plane в кнопку Telegram на страницах авторизации.
- [ ] Проверить тексты на соответствие правилам нейминга LeonidPro и @LeonidBot.
- [ ] Улучшить адаптивность и оформление виджета профиля.
- [ ] Использовать новые переменные брендинга в шаблонах.
- [ ] Доработать веб-интерфейс для задач, напоминаний, заметок, календаря и тайм-трекинга.
...
- [ ] Интегрировать тайм-трекер: команды `start/stop` в боте и отображение записей в веб-интерфейсе.
- [ ] Добавить интерфейсы для заметок и событий календаря (бот и веб).
...
- [ ] Интегрировать методы `WebUser` с системой Flask-Login во веб-интерфейсе.
...
- [x] Интегрировать тайм-менеджер с ботом и веб-интерфейсом.
...
- [x] Реализовать перенаправление заблокированных пользователей на страницу `ban.html`.
- [x] Добавить поиск и фильтрацию пользователей в админ‑панели.
- [ ] Проверка reCAPTCHA на сервере для форм авторизации.
...
- [ ] Дополнительные модули:
  - [x] Заметки — API и UI (см. `web/routes/notes.py`, `web/static/ui/notes.html`).
  ...
  - [x] Интеграция с календарями — Google Calendar (см. `web/routes/google_calendar.py`, `core/services/google_calendar_integration.py`).
