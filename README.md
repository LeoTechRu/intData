...
- См. в [AGENTS.md](./AGENTS.md#roles-charter--auto-switch-agx10) раздел Roles Charter & Auto-Switch (AGX/1.0) — устав ролей и протокол handoff.

## Как работать с AI-командой (для владельца продукта)

Вы общаетесь «как с бизнесом»: цель, ценность, ограничения. Первым всегда отвечает **тимлид** (роль TL): он уточнит противоречия, нарежет подзадачи, назначит роли, вернёт вам план с критериями приёмки.

**Шаблон запроса (минимум):**
- **Цель:** какую бизнес-ценность хотим получить и для кого.
- **Контекст:** что уже есть, что менять нельзя.
- **Ограничения:** сроки, риски, юридические/безопасность.
- **Признаки успеха (AC):** как поймём, что всё получилось.
- **Приоритет:** must-have / nice-to-have.

**Что будет дальше:** TL оформит `TaskCard`, заведёт ветки и назначит роли (backend/frontend/qa/techwriter/architect/devops). Команда относится к вашим идеям с профессиональным скепсисом: спорим аргументами, корректируем неверные допущения и всегда привязываем решения к AC/эпикам в этом файле. Прогресс и ссылки на PR видны в Agent Sync (AGENTS.md) и секциях Tasklist/Changelog.

**Пример: как это работает в живой сессии**

1. Вы даёте свободный текст «сделайте X».
2. TL отвечает первым, превращает его в TaskCard с AC и файлами, пишет:
   ```
   <<ROLE: backend>>
   # выполняю TaskCard TL-2025-09-19-...
   ```
3. Backend делает код в своей ветке, пушит, возвращает отчёт:
   - `handoff_to: teamlead`
   - `artifacts: PR #123, тесты зелёные, OpenAPI обновлён`
4. TL делает review, жмёт merge `feature/* → test`, триггерит smoke, затем fast-forward `test → main`, обновляет Tasklist/Changelog/Agent Sync.
5. Если нужны доки — TL оформляет TaskCard для техписателя и передаёт `<<ROLE: techwriter>>`.

## Оглавление
- [🤝 Как работать с AI-командой](#как-работать-с-ai-командой-для-владельца-продукта)
