...
- [ ] TL-2025-09-21-trivy-scan — Запустить CI workflow `Security Scan (Trivy)` и приложить отчёт `trivy-report.json` перед Gate-5 (owner: devops, см. `.github/workflows/trivy.yml`).
...
- [ ] P0•S — Убрать жёстко заданный маркер `'paid'` из обработчиков бота, вынести в конфиг/enum (см. InfoSec SHOULD).
