...
В @BotFather сделайте `/setdomain` → укажите домен, где открывается `/auth/login`.

`TELEGRAM_BOT_USERNAME` — без `@`.

Если видите «Bot domain invalid», значит домен страницы не совпадает с тем, что задан у бота, либо указан username с `@`.
