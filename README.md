...
- Любая новая информация (исследование, отчёт, ADR) добавляется в `docs/**` и линкуется из соответствующей секции README.
...
7. **Release / TL-Gate-4** — DevOps готовит fast-forward `test→main`, runbook и smoke.
8. **Documentation / TL-Gate-5** — Tech Writer синхронизирует README/Changelog, закрывает Agent Sync.

Каждый переход фиксируется `GateRecord` (UTC, подпись TL, риски). Детали и шаблоны находятся в `AGENTS.md`.
...
- **QA** — `tests/**`, `docs/reports/*tests*`, дефект-репорты, подтверждение AC.
- **InfoSec Advisory** — `docs/reports/infosec/**`, анализ зависимостей и кода.
- **DevOps/SRE** — `.github/**`, `config/**`, `docker-compose*`, observability, runbook релизов.
- **Tech Writer** — README, `docs/**`, Changelog, синхронизация funnel.
...
- **Документация:** все изменения отражаются в секциях ниже, ссылки на артефакты (`docs/reports`, ADR) обязательны.
...
- [x] TL-2025-09-19-test-runbook — Описать release-runbook `test → main` и smoke-чеклист (owner: codex, ветка `feature/E9/test-branch-deploy-codex`, документ `docs/runbooks/test-to-main.md`, README/Changelog обновлены; см. [E9](#e9-%D1%82%D0%B5%D1%81%D1%82%D1%8B-%D0%B8-%D0%B4%D0%BE%D0%BA%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D0%B0%D1%86%D0%B8%D1%8F-%D1%84%D0%B8%D1%87%D1%8D%D1%84%D0%BB%D0%B0%D0%B3)).
- [x] TL-2025-09-19-pytest-postgres-migration — Перевести диагностические и web-тесты на PostgreSQL, устранить `MissingGreenlet` и FK-конфликты (owner: codex, ветка `feature/E9/test-postgres-env-codex`; отчёт `docs/reports/2025-09-19-web-postgres-progress.txt`, follow-up: увеличить таймаут полного `pytest -q`).
- [x] TL-2025-09-19-pytest-postgres-qa — Подтвердить зелёный прогон тестов на PostgreSQL (owner: qa, отчёт `docs/reports/2025-09-20-pytest-postgres-qa.md`; результат: все группы пройдены партиями, полный `pytest -q` требует >10 мин).
- [x] TL-2025-09-19-ci-timeouts — Разбить `pytest` на два шага в CI и выгружать логи (owner: devops, см. записку `docs/reports/2025-09-20-ci-timeouts-analysis.md`).
