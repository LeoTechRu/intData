...
- **PARA‑first.** Проекты принадлежат **Областям**. Задачи и Ресурсы обязаны иметь **Project** или **Area**; при наличии проекта область наследуется автоматически. Быстрый ввод без контейнера падает в дефолтную область «Нераспределённое». Subjective overrides позволяют для конкретного пользователя привязать Project/Task/Resource к другой Area/Project без дублирования данных.
...
Pull/MR с тестами приветствуются. Смотрите [Changelog](./docs/CHANGELOG.md), [Backlog](./docs/BACKLOG.md) и [AGENTS.md](./AGENTS.md).
