...
## TL;DR
**AGENTS Spec v1.3 (2025-09-24), режим _Zero-Wait_:**
- Любое сообщение владельца **автоматически** запускает конвейер: TL-Router оформляет Intake/TaskCard и сам запускает **авто-переключение ролей** по путям без ожиданий.
- Конвейер: Intake → Архитектура (при необходимости) → Реализация → Merge в `test` → QA (path-guard) → InfoSec (advisory) → **DevOps (обязательный ребилд, рестарт, проверка логов)** → TL fast-forward `test->main` → Tech Writer.
- Эскалации к владельцу допускаются **только** при ODR-событиях (опасные миграции/данные, финансы/интеграции, CRITICAL/HIGH безопасность, SLA). Во всех других случаях агенты **принимают допущения** и доводят работу до релиза.
- Жёсткие anti-conflict-правила: path-locks в `agent_sync` + TTL, role-boundary-check, QA path-guard, auto-rebase перед merge, OpenAPI/SCHEMA sync, fast-forward релизы.
...
7. **DevOps Release / TL-Gate-5 (обязателен):** ребилд артефактов, миграции (идемпотентно), рестарт сервисов, **фиксация фрагментов логов без ошибок** и smoke-чеклист в GateRecord. Только после этого TL делает `fast-forward test->main`.
8. **Documentation / TL-Gate-6** - Tech Writer обновляет README/Changelog/Workflow, фиксирует ссылки и заключает итерацию.

## Owner Communication SLA (Zero-Wait)
- Любое сообщение владельца → **немедленный Intake/TL-Gate-0** и запуск конвейера. Вопросы владельцу запрещены, если есть разумные допущения.
- **ODR-события (только тогда спрашиваем):** необратимые DDL/массовые изменения данных; расходы/закупки/юридические риски; отключение/замена внешних интеграций; инциденты безопасности уровня CRITICAL/HIGH; изменение публичного SLA. Всё остальное – без ожиданий.
- Все принятые допущения фиксируются в README (Tasklist/Changelog) и в `agent_sync`.
...
- **SLA:** по любому новому сообщению владельца немедленно инициирует Intake/TL-Gate-0, оформляет intake и запускает декомпозицию без ожиданий дополнительных подтверждений.
...
## Branch & CI Guardrails (усилен для Zero-Wait)
...
- CI цепочка включает: `docs_folder_guard`, `readme_ru_check`, `links_check`, `role-boundary-check`, lint/test/build, OpenAPI/SCHEMA sync, `infosec-advisory` (semgrep/bandit/trivy), `release_e2e` (build → restart → logs-check) и release-runbook smoke.
...
## Sync Policy (Zero-Wait; README.md + AGENTS.md + agent_sync.yaml)
...
- После merge в `main` TL-bot запускает `scripts/sync-docs.sh` для зеркализации `test` и всех `feature/*`, а также фиксирует статус `Done` в `agent_sync` по затронутым путям (locks снимаются автоматически).

## Agent Sync & мультисессии (усилено)
...
- Завершая этап, выполняйте push, обновляйте статус, добавляйте GateRecord/hand-off и отражайте изменения в README/Changelog; **любой fail (тест/линт/scan/release) возвращает задачу к соответствующей роли** с минимальным диффом и автоматическим перезапуском конвейера.
...
agent_sync:
  ...
  - when_utc: "2025-09-23T21:12:15Z"
    agent: "codex"
    role: "fe"
    branch: "feature/E17/bitrix-leftnav"
    task: "TL-2025-09-23-bitrix-menu"
    epic_scope: "E17 / Frontend navigation"
    files:
      - "web/components/AppShell.tsx"
...
## Roles Charter & Auto-Switch (AGX/1.1 Zero-Wait)

**Правило ноль.** codex стартует в роли **Team Lead** и сам маршрутизирует между ролями до релиза, не обращаясь к владельцу, кроме случаев ODR.
...
- Любой fail (линт/тест/scan/release) → авто-возврат задачи к роли, ответственной за gate, с минимальным диффом и повторным запуском конвейера.
- Если TTL истёк или роль задержалась — TL-bot снимает бронь в `agent_sync` и перераспределяет задачу.
...
### Multi-agent Coordination (усилено)
...
- Резервируй задачи и файлы в [Agent Sync](#agent-sync) **обязательно**: укажи позывной, дату/время (UTC), ветку и ключевые файлы. Запрещено редактировать файл, занятый чужим lock; после merge/отмены работы снимай бронь.
...
- **Не ожидать владельца**, если нет ODR-события.
