...
## Owner Communication SLA (SLA взаимодействия с владельцем)
- Любое сообщение владельца считается сигналом к немедленному запуску Gate-0: тимлид сразу фиксирует intake, проверяет артефакты и переходит к декомпозиции без дополнительных подтверждений.
- После intake тимлид оперативно переключает роли и раздаёт TaskCard’ы — ожидание «ещё инструкции» от владельца запрещено.
- Дополнительные вопросы владельцу допустимы только в исключительных случаях, когда без уточнения нельзя корректно выполнить декомпозицию или понять целевую метрику задачи. Во всех остальных ситуациях агенты принимают допущения, документируют их в README/Agent Sync и продолжают работу.
