...
- Перед запуском новой сессии обязательно проверьте таблицу Agent Sync в `docs/BACKLOG.md` и закрепите задачу: укажите позывной, ветку `feature/<epic>/<scope>-<agent>`, список файлов и время в UTC. Без брони правки запрещены.
- Используйте lock-файлы или записи в Agent Sync для файлов: пока замок не снят, другие сессии не редактируют перечисленные пути. При завершении работы очистите запись. Такой подход соответствует рекомендациям систем координации MCP/Cursor и предотвращает конфликтное редактирование. citeturn0search0turn0reddit14
...
- В начале работы пересмотрите `docs/workflow.md` и следуйте описанному циклу: синхронизация → анализ `vision.md`/`conventions.md` → планирование через `update_plan` → исполнение → документация. Все временные договорённости фиксируйте в `idea.md` или `vision.md`, а принятые правила — в `conventions.md`.
- Перед коммитом убедитесь, что tasklist обновлён: отметьте выполненные пункты и добавьте ссылки на PR/коммиты. Это рационализирует распределение задач между агентами и соответствует лучшим практикам кооперативных LLM, где прозрачность намерений снижает риски дублирования работы. citeturn0search0
...
- `docs/workflow.md` — гайд по исполнению задач (порядок действий, эскалации, инциденты). При изменении процесса обновляйте этот файл и затем отражайте изменения в AGENTS.md.
- Пользователи должны запускать codex-cli согласно `docs/guides/codex-cli-multisession.md`; агенты обязаны ссылаться на этот гайд при формировании инструкций для людей.
...
- Резервируй задачи и файлы в [Agent Sync](./docs/BACKLOG.md#agent-sync): укажи позывной, дату/время (UTC), ветку и ключевые файлы. После merge/отмены работы снимай бронь.
